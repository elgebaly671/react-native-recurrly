...
const SubscriptionCard = ({ onPress,paymentMethod, expanded, name, price, status,startDate,currency, icon, billing, color, category, plan, renewalDate }: SubscriptionCardProps) => {
    return (
        <Pressable onPress={onPress}
            className={clsx('sub-card', expanded ? 'sub-card-expanded' : 'bg-card')} style={!expanded && color ? { backgroundColor: color } : undefined}>
                    ...
                    <View className='sub-details'>
                        ...
                        <View className='sub-row'>
                            <View className='sub-row-copy'>
                                ...
                                <Text className='sub-value' numberOfLines={1}
                                ellipsizeMode='tail'>{category?.trim()}</Text>
                            </View>
                        </View>
                        <View className='sub-row'>
                            <View className='sub-row-copy'>
